...
from apps.cmdb.collection.base import CollectBase, Collection, timestamp_gt_one_day_ago
...
class CollectK8sMetrics:
    ...
    def query_data(self):
        """查询数据"""
        sql = " or ".join(
            '{}{{instance_id="{}"}}'.format(j, self.cluster_name)
            for m in COLLECTION_METRICS.values()
            for j in m
        )
    ...
    def search_replicas(self):
        ...
        replicas_metrics = []
        sql = " or ".join(
            '{}{{instance_id="{}"}}'.format(m, self.cluster_name)
            for m in REPLICAS_METRICS
        )
        data = Collection().query(sql)
        ...
        for replicas_info in replicas_metrics:
            ...
            replicas_map.setdefault(replicas_key, {}).update(
                {replicas_key_name: replicas_info["index_value"]}
            )
    ...
    def format_annotation_metrics(metrics):
        ...
        try:
            if annotation["spec"]["template"]["metadata"].get("labels", False):
                labels_list = []
                for k, v in annotation["spec"]["template"]["metadata"][
                    "labels"
                ].items():
    ...
    def format_workload_metrics(self):
        ...
        for index_data in self.collection_metrics_dict["workload"]:
            if index_data["index_key"] == K8S_WORKLOAD_REPLICASET:
                ...
                # 单独存储注解指标
                index_data.update(
                    _annotation=self.format_annotation_metrics(index_data)
                )
        ...
        for annotations_info in annotations_metrics:
            ...
            annotations_map.setdefault(replicas_key, {}).update(
                {annotations_key_name: annotations_info["_annotation"]}
            )
        ...
        for workload_info in workload_metrics:
            ...
            if inst_name_key in REPLICAS_KEY:
                replicas = replicas_map.get(inst_name_key, {}).get(
                    workload_info[inst_name_key], 0
                )
            ...
            result.append(
                {
                    ...
                    "self_ns": namespace,
                    "labels": annotations_map.get(inst_name_key, {}).get(
                        workload_info[inst_name_key], ""
                    ),
                    "self_cluster": self.cluster_name,
                ...
                result.append(
                    {
                        ...
                        "name": name,
                        "labels": annotations_map.get(inst_name_key, {}).get(
                            rs_info[inst_name_key], ""
                        ),
                        "workload_type": workload_type,
                        "k8s_namespace": namespace,
                        "replicaset_name": rs_info[
                            "replicaset"
                        ],  # 保存ReplicaSet名称用于Pod关联
                        "self_ns": namespace,
                        "self_cluster": self.cluster_name,
                        "replicas": int(replicas),
                        "assos": [
    ...
    def format_pod_metrics(self):
        ...
        for index_data in self.collection_metrics_dict["pod"]:
            if index_data["index_key"] == K8S_POD_INFO:
                ...
                resource_limits[
                    (index_data["pod"], index_data["resource"])
    ...
    def format_node_metrics(self):
        ...
        for index_data in self.collection_metrics_dict["node"]:
            if index_data["index_key"] == K8S_NODE_INFO:
                ...
                inst_resource_dict[
                    (index_data["node"], index_data["resource"])
                ] = index_data["index_value"]
        result = []
        for inst_index_info in inst_index_info_list:
            ...
            info = dict(
                ...
                container_runtime_version=inst_index_info.get(
            ...
            disk = inst_resource_dict.get(
                (inst_index_info["node"], "ephemeral_storage")
            )
            if disk:
...
class CollectVmwareMetrics(CollectBase):
    ...
    def get_vm_asso(self, data, *args, **kwargs):
        result = []
        esxi_inst_name = self.model_resource_id_mapping["vmware_esxi"].get(
            data["vmware_esxi"], ""
        )
        if esxi_inst_name:
            result.append(
    ...
    def model_field_mapping(self):
        mapping = {
            "vmware_vc": {"vc_version": "vc_version", "inst_name": self.inst_name},
            "vmware_vm": {
                "inst_name": "inst_name",
                "ip_addr": "ip_addr",
    ...
    def prom_sql(self):
        sql = " or ".join(
            '{}{{instance_id="{}"}}'.format(m, f"{self.task_id}_{self.inst_name}")
    ...
    def format_metrics(self):
        ...
        for metric_key, metrics in self.collection_metrics_dict.items():
            ...
            if model_id == "vmware_vc":
                ...
                self.model_resource_id_mapping.update(
                    {model_id: {i["resource_id"]: i["inst_name"] for i in metrics}}
                )
...
class CollectNetworkMetrics(CollectBase):
    ...
    def format_data(self, data):
        ...
        for index_data in data["result"]:
            ...
            if "sysobjectid" in index_data["metric"]:
                ...
                if not oid_data:
                    logger.info(
                        "==OID does not exist, this instance data is skipped OID={}==".format(
                            oid
                        )
                    )
                    continue
        ...
        for metric_key, metrics in self.collection_metrics_dict.items():
            for index_data in metrics:
                if index_data["instance_id"] not in self.instance_id_map:
                    logger.info(
                        "This data is discarded because no feature library can be found for t"
                        "he OID. instance_id={}".format(index_data["instance_id"])
                    )
                    continue
    ...
    def find_interface_relationships(self, data):
        # 数据结构
        device_interfaces = defaultdict(
            dict
        )  # {instance_id: {ifindex: {"ifdescr": ..., "mac": ..., "ifalias": ...}}}
        ip_to_mac = defaultdict(dict)  # {instance_id: {ip: mac}}
        arp_table = defaultdict(
            dict
        )  # {instance_id: {ip: {"ifindex": ..., "mac": ...}}}
        ...
        for entry in data:
            ...
            if tag == "IFTable-IfDescr":  # 接口描述
                device_interfaces[instance_id].setdefault(ifindex, {})[
                    "ifdescr"
                ] = value
            elif tag == "IFTable-PhysAddress":  # 接口MAC地址
                device_interfaces[instance_id].setdefault(ifindex, {})[
                    "mac"
                ] = self.normalize_mac(value)
                ...
                arp_table[instance_id].setdefault(ifindex, {})[
                    "mac"
                ] = self.normalize_mac(value)
        ...
        for src_instance, src_arp in arp_table.items():
            for ip, arp_info in src_arp.items():
                ...
                if dst_mac in mac_to_device:
                    ...
                    if (
                        dst_instance not in self.instance_id_map
                        or src_instance not in self.instance_id_map
                    ):
                        logger.info(
                            "This data is discarded because no feature library "
                            "can be found for the OID. instance_id={}".format(
                                src_instance
                            )
                        )
                        continue
                    ...
                    relations.append(
                        {
                            ...
                            "source_inst_name": self.set_interface_inst_name(
                                data={
                                    "instance_id": src_instance,
                                    **self.set_alias_descr(src_interface),
                                }
                            ),
                            ...
                            "target_inst_name": self.set_interface_inst_name(
                                data={
                                    "instance_id": dst_instance,
                                    **self.set_alias_descr(dst_interface),
                                }
                            ),
                            "model_id": "interface",
                            "asst_id": "connect",
...
class MiddlewareCollectMetrics(CollectBase):
    ...
    def _metrics(self):
        assert (
            self.model_id in MIDDLEWARE_METRIC_MAP
...
class DBCollectCollectMetrics(CollectBase):
    ...
    def _metrics(self):
        assert (
            self.model_id in DB_COLLECT_METRIC_MAP
        ), f"{self.model_id} needs to be defined in DB_COLLECT_METRIC_MAP"
